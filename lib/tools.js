...
export const TOOL_RISK = {
  ...
  task_write: RISK.SAFE,
  task_append: RISK.SAFE,
...
export const FUNCTION_TOOLS = [
  ...
  },
  {
    ...
    name: "task_write",
    description:
      "Перезаписать файл задачи целиком. Файл задачи — твоя постоянная память по этому чату: он переживает сжатие контекста " +
      "и перезапуск браузера, и его содержимое всегда показывается тебе в системном промпте. " +
      "Пиши markdown: цель задачи, что уже сделано, найденные результаты со ссылками, что осталось. " +
      "Используй в начале работы (записать цель и план) и когда файл нужно переструктурировать или ужать.",
    parameters: {
      type: "object",
      properties: {
        content: {
          type: "string",
          description: "Полное новое содержимое файла задачи в markdown.",
        },
      },
      required: ["content"],
      additionalProperties: false,
  ...
  },
  {
    ...
    name: "task_append",
    description:
      "Дописать блок в конец файла задачи, ничего не стирая. Основной способ копить находки: " +
      "каждую найденную ссылку, товар, цену, факт сразу добавляй сюда — тогда они не потеряются при сжатии контекста. " +
      "Пиши компактно: строка-другая на находку, обязательно с URL.",
    parameters: {
      type: "object",
      properties: {
        content: {
          type: "string",
          description: "Текст (markdown), который добавится в конец файла.",
        },
      },
      required: ["content"],
      additionalProperties: false,
...
// Нативный веб-поиск Responses API — исполняется провайдером внутри вызова модели
export const WEB_SEARCH_TOOL = { type: "web_search" };

// Наш прокси-тул: основная модель просит найти, а background выполняет запрос
// отдельной search-моделью (settings.searchModel) с нативным web_search.
export const WEB_SEARCH_FUNCTION = {
  type: "function",
  name: "web_search",
  description:
    "Найти информацию в интернете. Поиск выполняет отдельная поисковая модель и возвращает готовую текстовую выжимку со ссылками. " +
    "Формулируй запрос так, как искал бы человек, и добавляй в него весь нужный контекст — поисковая модель не видит наш диалог и страницу. " +
    "Используй для свежих данных, цен, новостей, фактов, которых нет на текущей странице.",
  parameters: {
    type: "object",
    properties: {
      query: {
        type: "string",
        description:
          "Самодостаточный поисковый запрос со всем контекстом. Язык — тот, на котором вероятнее найдётся ответ.",
      },
    },
    required: ["query"],
    additionalProperties: false,
...
// searchModel задана — поиск уходит отдельной модели через function-tool,
// иначе оставляем нативный web_search в основном запросе (старое поведение).
export function buildTools({ webSearch = true, searchModel = "" } = {}) {
  if (!webSearch) return [...FUNCTION_TOOLS];
  return searchModel
    ? [...FUNCTION_TOOLS, WEB_SEARCH_FUNCTION]
    : [...FUNCTION_TOOLS, WEB_SEARCH_TOOL];
}
...
export function describeAction(name, args) {
  switch (name) {
    ...
    case "task_write":
      return "Перезаписать файл задачи";
    case "task_append":
      return "Дописать в файл задачи";
